...
export default function ConfirmBookingDialog({
  ...
  const { mutate, isPending, isError, error, reset } = useMutation({
    mutationFn: (data: IAppointmentPayload) =>
      appointmentService.makeAppointment(data),
    onSuccess: () => {
      closeDialog();
  ...
  return (
    <Dialog open={!!bookingDateTime} onClose={closeDialog}>
      <Box display="flex">
      ...
      <DialogContent>
        ...
        <Typography>
          {'Time: ' + bookingDateTime?.getHours() + ':00'}
        </Typography>
      </DialogContent>
      <LoadingButton loading={isPending} onClick={confirmAppointment}>
        Confirm
      </LoadingButton>
    </Dialog>
  );
