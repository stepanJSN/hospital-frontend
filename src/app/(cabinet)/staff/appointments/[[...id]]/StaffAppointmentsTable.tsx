...
type StaffAppointmentsTable = {
  data: IAppointment[];
  changeAppointmentStatus: ({ id, status }: { id: string, status: boolean }) => void;
  setAppointmentId: (id: string) => void;
}

export default function StaffAppointmentsTable({ data, setAppointmentId, changeAppointmentStatus }: StaffAppointmentsTable) {
  return (
    <DataTable
      ...
      columns={[
        ...
        { header: 'Name Surname', accessor: (row) => (
          <UILink component={Link} href={`/customers/${row.customer.id}`}>{`${row.customer?.name} ${row.customer?.surname}`}</UILink>
        )},
