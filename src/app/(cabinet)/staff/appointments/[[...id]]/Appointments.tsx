...
export default function Appointments({ staffId }: AppointmentsProps) {
  ...
  const [filterData, setFilterData] = useState<IGetStaffAppointmentsForm>({
    isCompleted: false,
  });
  ...
  const { refetch, data, isFetching, isError, isSuccess } = useQuery({
    ...
    queryFn: async () =>
      appointmentService.getByUserId(
        { returnType: 'customer', ...filterData },
        staffId,
      ),
  ...
  const {
    ...
    mutationFn: (data: { id: string; status: boolean }) =>
      appointmentService.changeStatus(data.id, { isCompleted: data.status }),
    onSettled: () => refetch(),
  });
  ...
  return (
    <Box marginY={2} marginX={1}>
      <Box display="flex" justifyContent="space-between">
        ...
        {isSuccess && (
          <ExportExcel data={mapData(data)} fileName="appointments" />
        )}
      </Box>
      ...
      {isChangeStatusError && (
        <Notification text="Error. Failed to change the status of the appointment" />
      )}
    </Box>
  );
}
