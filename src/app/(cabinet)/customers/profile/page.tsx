...
export default function Profile() {
  ...
  const { control, handleSubmit, setValue } = useForm<UpdateUser>();
  ...
  const {
    ...
    onSuccess: (data: IUpdateAvatarResponse) =>
      queryClient.setQueryData(['profile'], (oldProfileData: IUser) =>
        oldProfileData
          ? {
              ...oldProfileData,
  ...
  const onSubmit: SubmitHandler<UpdateUser> = (data) =>
    mutate(replaceEmptyFieldsWithNull(data));
  const handleClose = () => {
  ...
  return (
    <Box flex="auto" display="flex" justifyContent="center" mt={2}>
      {isProfileSuccess && (
        <Box
          component="form"
        ...
        >
          ...
          {isSuccess && (
            <Alert severity="success">Profile info was updated</Alert>
          )}
          {isError && (
            <Alert severity="error">{isError && 'Error. Try again'}</Alert>
          )}
          <FormInput
            label="Name"
            control={control}
      ...
      {isProfilePending && (
        <CircularProgress size={65} sx={{ position: 'absolute', top: '40%' }} />
      )}
      {isProfileDataError && (
        <Typography color="error" position="absolute" top="30%" fontSize={20}>
      ...
      {isAvatarMutationError && (
        <Notification text="Error. Unable to update avatar" />
      )}
